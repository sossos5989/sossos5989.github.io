...
### 알고리즘 설계
1. $block$의 $0\degree, 90\degree, 180\degree, 270\degree$를 미리 구해놓습니다. 가장 위쪽의 왼쪽 블록이 $(0,0)$ 좌표라고 생각하면, 나머지 연결된 블록들을 **상대 좌표**로 표현합니다. 이를 통해 **중복되는 블록**(회전해도 같은 모양)을 지울 수 있습니다.
2. 알고리즘을 구현하기 위해 연산에 필요한 추가적인 값을 구합니다.
   - 비어 있는 칸 `covered[i][j] == 0`의 위치를 `vector<pair<int,int>> emptyCells`에 저장합니다.
   - 전체 빈 칸의 개개수를 `int leftCells` 변수에 저장하여 가지치기 조건에 활용합니다.
...
## 알고리즘 구현
### 블록의 회전 구현
블록의 $90\degree$회전을 구현하는 함수를 작성합니다.
```c++
